          ...
          return (
            <button
              ...
              className={`inline cursor-pointer rounded border-b-2 px-0.5 font-medium
                transition-colors duration-100 ${v.text}
                ${selectedIdx === item.citationIdx
                  ? `${v.bg} shadow-sm`
                  : "border-current/40 hover:bg-ink/[0.06]"}`}
            >
              {item.text}
            </button>
          );
...
function TopBar({ result, copied, exporting, onCopy, onDownload }: {
  ...
  return (
    <div className="flex shrink-0 flex-wrap items-center gap-3 border-b border-paper-fixed/10 bg-ink-fixed px-4 py-2.5">
      <Link to="/" className="inline-flex items-center gap-1.5 text-sm font-semibold text-paper-fixed transition-colors hover:text-accent-lime">
        <ArrowLeft className="h-4 w-4" aria-hidden="true" />
        New document
      </Link>
      <span className="text-paper-fixed/20">|</span>
      <span className="font-mono text-[11px] text-paper-fixed/55">{result.total_citations} citations</span>
      {n_fabricated > 0 && (
        <span className="flex items-center gap-1.5 font-mono text-[11px] font-semibold text-paper-fixed/85">
          <span className="h-1.5 w-1.5 rounded-full bg-bad-bd" /> {n_fabricated} fabricated
        </span>
      )}
      {n_misapplied > 0 && (
        <span className="flex items-center gap-1.5 font-mono text-[11px] font-semibold text-paper-fixed/85">
          <span className="h-1.5 w-1.5 rounded-full bg-warn-bd" /> {n_misapplied} misapplied
        </span>
      )}
      {n_verified > 0 && (
        <span className="flex items-center gap-1.5 font-mono text-[11px] text-paper-fixed/85">
          <span className="h-1.5 w-1.5 rounded-full bg-good-bd" /> {n_verified} verified
        </span>
      )}
      <span className="ml-auto font-mono text-[11px] text-paper-fixed/45">{result.processing_ms} ms</span>
      <button
        type="button"
        onClick={onDownload}
        disabled={exporting}
      ...
      >
        {copied ? <Check className="h-3 w-3 text-accent-lime" aria-hidden="true" /> : <Copy className="h-3 w-3" aria-hidden="true" />}
